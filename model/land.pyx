...
cdef void land_tri_dealloc(LandTri* tri):
  if land_tri_has_child(tri):
    land_tri_dealloc(tri.left_child)
    land_tri_dealloc(tri.right_child)
  free(tri)

cdef int land_tri_has_child(LandTri* tri):
  return tri.left_child != NULL

cdef int land_tri_diamond(LandTri* tri):
  return (tri.base_neighbor == NULL) or ((tri.v2 == tri.base_neighbor.v3) and (tri.v3 == tri.base_neighbor.v2))

cdef void land_tri_sphere(float r[4], float* p1, float* p2, float* p3):
...
cdef LandTri* land_get_tri():
  # XXX try to re-use LandTri structures
  return <LandTri*> malloc(sizeof(LandTri))

cdef void land_tri_drop(LandTri* tri):
  # XXX try to re-use LandTri structures
  free(tri)
  
cdef LandTri* land_create_tri(LandVertex* v1, LandVertex* v2, LandVertex* v3, LandPatch* patch):
  cdef LandTri* tri
  tri               = land_get_tri()
  tri.v1            = v1
  tri.v2            = v2
  ...
  land_tri_sphere (tri.sphere, v1.coord, v2.coord, v3.coord)
  face_normal     (tri.normal, v1.coord, v2.coord, v3.coord)
  vector_normalize(tri.normal)
  return tri

cdef LandTri* land_create_child_tri(LandVertex* v1, LandVertex* v2, LandVertex* v3, LandTri* parent):
  cdef LandTri* tri
  tri             = land_get_tri()
  tri.v1          = v1
  tri.v2          = v2
...
cdef void land_tri_update_neighbor_after_split (LandTri* tri):
  tri.left_child.left_neighbor    = tri.right_child
  if (tri.left_neighbor != NULL) and land_tri_has_child(tri.left_neighbor):
  ...
  if (tri.right_neighbor != NULL) and land_tri_has_child(tri.right_neighbor):
...
cdef void land_tri_force_split(LandTri* tri, LandVertex* apex):
  tri.left_child  = land_create_child_tri(apex, tri.v1, tri.v2, tri)
  tri.right_child = land_create_child_tri(apex, tri.v3, tri.v1, tri)
  land_tri_update_neighbor_after_split(tri)
  
cdef void land_tri_update_neighbor_after_merge (LandTri* tri):
  if tri.left_neighbor != NULL:
    if tri.left_neighbor.v1 == tri.v1:
      tri.left_neighbor.right_neighbor = tri
      if land_tri_has_child(tri.left_neighbor): tri.left_neighbor.right_child.base_neighbor = tri
    else:
  ...
  if tri.right_neighbor != NULL:
    if tri.right_neighbor.v1 == tri.v1:
      ...
      if land_tri_has_child(tri.right_neighbor):
...
cdef void land_get_height_at_factors(LandVertex* v1, LandVertex* v2, LandVertex* v3, float x, float z, float* k, float* t):
...
cdef void land_drawColor_radeon(float* vect):
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, vect)
  
cdef void noop():
  pass

cdef void land_disableColor_radeon():
  #glColor4fv(white) # XXX really needed ?
  glDisable(GL_COLOR_MATERIAL)
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, white)
  
cdef void land_enableColor_radeon():
...
cdef land_drawColor_FUNC    land_drawColor
cdef land_disableColor_FUNC land_disableColor
cdef land_enableColor_FUNC  land_enableColor

land_drawColor    = glColor4fv
land_disableColor = noop
land_enableColor  = noop


# XXX inline this
cdef void SET_COLOR_OPAQUE(_Land self, int index):
  if self._colors == NULL: land_drawColor(renderer.current_material._diffuse)
  else:                    land_drawColor(self._colors + index)

# XXX inline this
cdef void SET_COLOR_ALPHA(_Land self, int index):
  cdef float  ccc[4]
  cdef float* color
...
cdef class _Land(CoordSyst):
  ...
  property patch_size:
    ...
    def __set__(self, x):
      self._patch_size = x
      if self._option & LAND_INITED: self._option = self._option - LAND_INITED
      
  property has_vertex_options:
    def __get__(self):
      return self._option & LAND_VERTEX_OPTIONS
  ...
  property scale_factor:
    ...
    def __set__(self, x):
      ...
      self._option = self._option & ~LAND_INITED
  ...
  property raypick_with_LOD:
    def __get__(self):
      return self._option & LAND_REAL_LOD_RAYPICK
    def __set__(self, x):
      if x: self._option = self._option |  LAND_REAL_LOD_RAYPICK
      else: self._option = self._option & ~LAND_REAL_LOD_RAYPICK
  ...
  cdef void _check_size(self):
    ...
    while 1:
      ...
      if self._nb_vertex_width <= d:
        if self._nb_vertex_width < d: print "WARNING Land size width must be (2^n) + 1"
        break
      i = i + 1
    ...
    while 1:
      ...
      if self._nb_vertex_depth <= d:
        if self._nb_vertex_depth < d: print "WARNING Land size depth must be (2^n) + 1"
        break
      i = i + 1
  ...
  def __init__(self, _World parent = None, int width = 0, int depth = 0, float scale_factor = 1.0):
    ...
    if (width != 0) and (depth != 0):
      ...
      self._check_size()
      self._vertices = <LandVertex*> malloc(nb * sizeof(LandVertex))
      for i from 0 <= i < nb:
        v = self._vertices + i
        v.coord[1] = 0
  ...
  cdef void _free_patchs(self):
    cdef LandPatch* patch
    cdef int        i
    for i from 0 <= i < self._nb_patchs:
      patch = self._patchs + i
      land_tri_dealloc(patch.tri_top)
      land_tri_dealloc(patch.tri_left)
      land_tri_dealloc(patch.tri_right)
      land_tri_dealloc(patch.tri_bottom)
    free(self._patchs)
    self._patchs = NULL
  ...
  def set_material_layer_angle(self, _Material material, float start, float end, float angle_min, float angle_max):
    ...
    if not(self._option & LAND_INITED): self._init()
...
Removes all colors, from all vertices that have colors."""
    ...
    self._option = self._option & (~LAND_COLORED)
    
  def get_vertex_option(self, int x, int z):
    """Land.get_vertex_option(X, Z) => (HIDDEN, NON_SOLID, FORCE_PRESENCE)
...
See Land.set_vertex_option for the available options."""
    ...
    return option & LAND_VERTEX_HIDDEN, option & LAND_VERTEX_NON_SOLID, option & LAND_VERTEX_FORCE_PRESENCE
  
  def set_vertex_option(self, int x, int z, int hidden = 0, int non_solid = 0, int force_presence = 0):
    """Land.set_vertex_option(X, Z, HIDDEN, NON_SOLID, FORCE_PRESENCE)
...
"""
    ...
    self._check_vertex_options()
    self._vertex_options[index] = (self._vertex_options[index] & ~(LAND_VERTEX_HIDDEN | LAND_VERTEX_NON_SOLID | LAND_VERTEX_FORCE_PRESENCE)) | (hidden * LAND_VERTEX_HIDDEN) | (non_solid * LAND_VERTEX_NON_SOLID) | (force_presence * LAND_VERTEX_FORCE_PRESENCE)
  ...
  def set_vertex_color(self, int x, int z, color):
    ...
    if   1.0 - ccolor[3] > EPSILON:          self._vertex_options[n] = self._vertex_options[n] |  LAND_VERTEX_ALPHA
    elif self._option & LAND_VERTEX_OPTIONS: self._vertex_options[n] = self._vertex_options[n] & ~LAND_VERTEX_ALPHA
    if   ccolor[3] < EPSILON:                self._vertex_options[n] = self._vertex_options[n] |  LAND_VERTEX_HIDDEN
    elif self._option & LAND_VERTEX_OPTIONS: self._vertex_options[n] = self._vertex_options[n] & ~LAND_VERTEX_HIDDEN
...
(e.g. with Land.set_height)."""
  ...
  cdef void _init(self):
    ...
    self._create_patchs()
    if self._option & LAND_VERTEX_OPTIONS: self._force_presence()
    self._option = self._option | LAND_INITED
  ...
  cdef void _create_patch(self, LandPatch* patch, int x, int z, int patch_size):
    ...
    cdef float*      coords
    cdef LandVertex* vertex, *center, *v0, *v1, *v2, *v3
    patch.level = 0
    x1 =  x      * patch_size
    x2 = (x + 1) * patch_size
    z1 =  z      * patch_size
    ...
    center = v0 + ((v3 - v0) >> 1)
    patch.tri_top    = land_create_tri(center, v1, v0, patch)
    patch.tri_left   = land_create_tri(center, v0, v2, patch)
    patch.tri_right  = land_create_tri(center, v3, v1, patch)
    patch.tri_bottom = land_create_tri(center, v2, v3, patch)
  ...
  cdef void _create_patchs(self):
    ...
    if self._patchs != NULL: land_free_patchs(land)
    self._patchs = <LandPatch*> malloc(self._nb_patchs * sizeof(LandPatch))
    k = 0
  ...
  def compute_shadow_color(self, _Light light, shadow_color):
    ...
    self._colors = NULL
    self._nb_colors = 0
    if not(self._option & LAND_COLORED):
      self._option = self._option | LAND_COLORED
  ...
  cdef void _tri_split(self, LandTri* tri):
    cdef LandVertex* v
    if not land_tri_diamond(tri): self._tri_split(tri.base_neighbor)
    ...
    tri.left_child  = land_create_child_tri(v, tri.v1, tri.v2, tri)
    tri.right_child = land_create_child_tri(v, tri.v3, tri.v1, tri)
    if tri.base_neighbor != NULL: land_tri_force_split(tri.base_neighbor, v)
    land_tri_update_neighbor_after_split(tri)
  ...
  cdef int _tri_merge_child(self, LandTri* tri):
    ...
    if land_tri_has_child(tri.left_child ) and not self._tri_merge_child(tri.left_child ): return 0
    if land_tri_has_child(tri.right_child) and not self._tri_merge_child(tri.right_child): return 0
    if base != NULL:
      if land_tri_has_child(base.left_child ) and not self._tri_merge_child(base.left_child ): return 0
      if land_tri_has_child(base.right_child) and not self._tri_merge_child(base.right_child): return 0
      
    if (self._option & LAND_VERTEX_OPTIONS) and (self._vertex_options[tri.left_child.v1 - self._vertices] & LAND_VERTEX_FORCE_PRESENCE):
      return 0
    
    land_tri_update_neighbor_after_merge(tri)
    if base != NULL:
      land_tri_update_neighbor_after_merge(base)
      land_tri_drop(base.left_child)
      land_tri_drop(base.right_child)
      base.left_child = base.right_child = NULL
      
    land_tri_drop(tri.left_child )
    land_tri_drop(tri.right_child)
  ...
  cdef void _force_presence(self):
    ...
    for j from 0 <= j < self._nb_vertex_depth:
      for i from 0 <= i < self._nb_vertex_width:
        if self._vertex_options[i + j * self._nb_vertex_width] & LAND_VERTEX_FORCE_PRESENCE:
  ...
  cdef void _tri_batch(self, LandTri* tri, Frustum* frustum):
    ...
    if sphere_in_frustum(frustum, tri.sphere):
      if land_tri_has_child(tri): # recurse in children
        self._tri_batch(tri.left_child,  frustum)
        self._tri_batch(tri.right_child, frustum)
      else:
        if self._option & LAND_VERTEX_OPTIONS:
          ...
          if ((o1 & LAND_VERTEX_HIDDEN) and
              (o2 & LAND_VERTEX_HIDDEN) and
              (o3 & LAND_VERTEX_HIDDEN)): return
          if (o1 & LAND_VERTEX_ALPHA) or (o2 & LAND_VERTEX_ALPHA) or (o3 & LAND_VERTEX_ALPHA):
  ...
  cdef void _vertex_render_special(self, LandVertex* vertex):
    ...
    if (self._option & LAND_VERTEX_OPTIONS) and (self._vertex_options[index] & LAND_VERTEX_ALPHA) and (not (<_Material> (vertex.pack.material_id)) is renderer.current_material):
      ccc[0] = color[0]
      ccc[1] = color[1]
      ccc[2] = color[2]
      ccc[3] = 0.0
      land_drawColor (ccc)
    else: land_drawColor(color)
  ...
  cdef void _tri_render_secondpass(self, LandTri* tri):
    ...
    if ((not(self._option & LAND_VERTEX_OPTIONS) or
         not((self._vertex_options[tri.v1 - self._vertices] & LAND_VERTEX_ALPHA) or
             (self._vertex_options[tri.v2 - self._vertices] & LAND_VERTEX_ALPHA) or
             (self._vertex_options[tri.v3 - self._vertices] & LAND_VERTEX_ALPHA))) and
  ...
  cdef void _render(self, CoordSyst coordsyst):
    ...
    #glInterleavedArrays(GL_T2F_N3F_V3F, sizeof(LandVertex), self._vertices)
    
    # Renderer.data is like :
    #   [pack1*, land_tri1*, land_tri2*,..., NULL, pack2*, land_tri3,..., NULL, NULL]
    ...
    if   renderer.state == RENDERER_STATE_OPAQUE:
      while pack:
        ...
        tri = <LandTri*> chunk_get_ptr(renderer.data)
        while tri:
          #index = tri.v1 - self._vertices
          if self._colors != NULL: land_drawColor(self._colors + index)
          #glArrayElement(index)
          glTexCoord2fv(tri.v1.texcoord)
          glNormal3fv(tri.v1.normal)
          glVertex3fv(tri.v1.coord)
  ...
  cdef void _tri_raypick(self, LandTri* tri, float* raydata, RaypickData data):
    ...
    if land_tri_has_child(tri):
      ...
      if ((self._option & LAND_VERTEX_OPTIONS) and 
          (self._vertex_options[tri.v1 - self._vertices] & LAND_VERTEX_NON_SOLID) and
          (self._vertex_options[tri.v2 - self._vertices] & LAND_VERTEX_NON_SOLID) and
          (self._vertex_options[tri.v3 - self._vertices] & LAND_VERTEX_NON_SOLID)): return
  ...
  cdef void _full_raypick(self, LandVertex* v1, LandVertex* v2, LandVertex* v3, float* normal, float* raydata, RaypickData data):
    ...
    if ((not(self._option & LAND_VERTEX_OPTIONS)) or 
        (not(self._vertex_options[v1 - self._vertices] & LAND_VERTEX_NON_SOLID)) or
        (not(self._vertex_options[v2 - self._vertices] & LAND_VERTEX_NON_SOLID)) or
        (not(self._vertex_options[v3 - self._vertices] & LAND_VERTEX_NON_SOLID))):
      coord1 = v1.coord
      coord2 = v2.coord
      coord3 = v3.coord
  ...
  cdef void _raypick(self, RaypickData raypick_data, CoordSyst raypickable):
    ...
    if self._option & LAND_REAL_LOD_RAYPICK:
      for i from 0 <= i < self._nb_patchs:
        patch = self._patchs + i
  ...
  cdef int _tri_raypick_b(self, LandTri* tri, float* raydata, int option):
    ...
    if land_tri_has_child(tri):
      ...
      if self._option & LAND_VERTEX_OPTIONS:
        if ((self._vertex_options[tri.v1 - self._vertices] & LAND_VERTEX_NON_SOLID) and
            (self._vertex_options[tri.v2 - self._vertices] & LAND_VERTEX_NON_SOLID) and
            (self._vertex_options[tri.v3 - self._vertices] & LAND_VERTEX_NON_SOLID)): return 0
    ...
    if self._option & LAND_REAL_LOD_RAYPICK:
      ...
      for i from 0 <= i < self._nb_patchs:
        ...
        if (v1.coord[0] <= x <= v2.coord[0]) and (v1.coord[2] <= z <= v2.coord[2]):
          ...
          tri = patch.tri_top
          for j from 0 <= j < 4:
            land_get_height_at_factors(tri.v1, tri.v2, tri.v3, x, z, &k, &t)
            ...
            tri = tri + 1 # HACK : patch.tri_top, patch.tri_left, patch.tri_right, patch.tri_bottom are in this order in the LandPatch structure.
      ...
      if (0 <= ix < self._nb_vertex_width) and (0 <= iz < self._nb_vertex_depth):
        if not((ix + iz) & 1): # if both ix and iz are odd or even
          if nz - iz > nx - ix:
            ...
            land_get_height_at_factors(v1, v2, v3, x, z, &k, &t)
  ...
  cdef __getcstate__(self):
    ...
    chunk = get_chunk()
    chunk_add_int_endian_safe   (chunk, self._option & ~LAND_INITED)
    ...
    if (self._option & LAND_COLORED) and (self._nb_colors > 0):
      chunk_add_floats_endian_safe(chunk, self._colors        , 4 * self._nb_colors)
      chunk_add_ints_endian_safe  (chunk, self._vertex_colors , nb)
      
    if self._option & LAND_VERTEX_OPTIONS: chunk_add_chars_endian_safe(chunk, self._vertex_options, nb)
  ...
  cdef void _check_vertex_options(self):
    if not(self._option & LAND_VERTEX_OPTIONS):
      self._option = self._option | LAND_VERTEX_OPTIONS
      #free(self._vertex_options)
