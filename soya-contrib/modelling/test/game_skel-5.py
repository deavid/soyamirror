...
sys.path.append('/home/dunk/cvs/soya-contrib/modelling/')

from modelling import bbox
...
class Character(soya.World):
  ...
  def begin_action(self, action):
    ...
    r = context.raypick(new_center, self.down, 0.1 + self.radius_y, 0, 0)
    ...
    for vec in (self.left, self.right, self.front, self.back, self.up):
      r = context.raypick(new_center, vec, self.radius, 0, 0)
      if r:
...
b= bbox.BoundingBox(level, character.perso.shape)
b.move(character)
