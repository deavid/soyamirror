...
def setScene(newscene):
    print "gameobjects newscene = ", newscene
    global CurrentScene = newscene
    print "gameobjects CurrentScene = ",CurrentScene
"""
    
def getScene():
    """ helper function to return current scene object """
    #print soya.IDLER.scenes[:]
    return soya.IDLER.scenes[0]
     
class Objects:   
    
    def base_object(self):
        obj = {'objid':'none', 'parent':'scene', 'type':'None', 'position':(0,0,0)}
        return obj
        
class ClientLevel(soya.World):
  """ The base class for all levels """

  def __init__(self, parent, objectinfo):
    
    soya.World.__init__(self)
    self.objinfo = objectinfo
    self.size_y= self.objinfo['size'][1]
    self.size_x= self.objinfo['size'][0]
    sun = soya.Light(self)
    sun.directional = 1
...
class Level(soya.World):
  ...
  def create(self):
    ...
    objectlist = []
    levelobject = self.Objects.base_object()
    levelobject['objid'] = 'level1'
    levelobject['type'] = 'ClientLevel'
    levelobject['parent'] = 'scene'
    levelobject['size'] = (10, 8)
    objectlist.append(levelobject)
    ...
    for r, x in enumerate(range(-self.size_x,self.size_x-1)):
      for c, y in enumerate(range(-self.size_y,self.size_y-1)): 
        OBJECT = self.Objects.base_object()
        if (r % 2 == 0 and c % 2 == 0) or \
           (r == 0 or c == 0 or r == border_row  or c == border_col ):
            # This is a wall block
            myid = 'wall_' + str(x) +str(y)
            OBJECT['objid'] =  myid
            OBJECT['type'] = 'Cube'
            OBJECT['parent'] = 'level1'
            OBJECT['position'] = (x,1,y)
            OBJECT['mapposition'] = (r,c)
          ...
          if (x, y) in always_clear:
            ...
            # A soft block
            myid = 'softblock_' + str(x) +str(y)
            OBJECT['objid'] =  myid
            OBJECT['type'] = 'SoftCube'
            OBJECT['parent'] = 'level1'
            OBJECT['position'] = (x,1,y)
            OBJECT['mapposition'] = (r,c)
            objectlist.append(OBJECT)
...
class MapObject(soya.Volume):
  def __init__(self, parent, objinfo,shape):
    soya.Volume.__init__(self, parent, shape)
    self.objinfo = objinfo
    self.set_xyz(*self.objinfo['position'])
    
    try:
      self.parent.map[int(round(self.x))][int(round(self.z))] = self
    except:
      print self.parent
      traceback.print_exc()
      sys.exit(1)
...
class SoftCube(MapObject):
  ...
  def __init__(self,parent, objinfo):
    ...
    MapObject.__init__(self, parent, objinfo, shape)
...
class Character(soya.World):
  ...
  def createCal3dVolume(self):
    character=soya.Cal3dShape.get(self.objinfo['mesh'])

    self.perso=soya.Cal3dVolume(self,character)
...
class Player(Character):
  ...
  def begin_action(self, action):
    ...
    #print getScene()
    print "newcenter is ", new_center
    #print "self.down is ", self.down
    
    r = context.raypick(new_center, self.down, 1.1 + self.radius_y, 0)
    print "r = ", r
    print self.jumping
    if r and not self.jumping:
      print "inside the if r block:"
    ...
    try:
      ...
      print self.parent.map[rx+self.parent.size_x][rz+self.parent.size_y]
      print rx, rz
    except:
      traceback.print_exc()
      print self.parent
