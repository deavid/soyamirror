...
# shader-3: shader's parameters

# Using shader's parameters, it is possible to use the same shader with different results.
# In this lesson, we are going to add a color parameter to the shader-2.py tutorial.
...
sword1.rotate_z(-90.0)

sword2 = soya.Body(scene, sword_model)
sword2.x =  0.0
sword2.y = -1.0
sword2.rotate_y( 90.0)
sword2.rotate_z(-90.0)

sword3 = soya.Body(scene, sword_model)
sword3.x =  1.0
sword3.y = -1.0
sword3.rotate_y( 90.0)
sword3.rotate_z(-90.0)
...
MUL color2, program.local[1], sincos.x;
...
# Set the same shader on the 3 swords, but with different parameters.
# local1 correspond to "program.local[1]" in the shader's code.
# NB remind that program.local[0] is time, and is managed by Soya !

sword1.add_deform(shader(local1 = [0.0, 1.0, 0.0, 1.0]))
sword2.add_deform(shader(local1 = [0.0, 0.0, 1.0, 1.0]))
sword3.add_deform(shader(local1 = [1.0, 0.0, 0.0, 1.0]))

# Parameters can also be changed by hand as following :
#
#    deform = shader(local1 = [0.0, 1.0, 0.0, 1.0])
#    sword1.add_deform(deform)
#    deform.params[1] = [0.0, 1.0, 0.3, 1.0]
