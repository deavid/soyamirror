...
cdef class _SolidShape(_SimpleShape):
	cdef void _render(self, CoordSyst coord_syst):
		...
		if pos[2] + self._sphere[3] < -renderer.current_camera._front:
			...
			_SimpleShape._render(self, coord_syst)
	...
	cdef void _render_triangle_solid(self, ShapeFace* face, CoordSyst coord_syst, int vertex_indices[3]):
		...
		for i from 0 <= i < 3:
			...
			if self._option & SHAPE_DIFFUSES : glColor4fv   (self._colors   + self._vertex_diffuses [vertex_indices[i]])
			if self._option & SHAPE_EMISSIVES: glMaterialfv (GL_FRONT_AND_BACK, GL_EMISSION, self._colors + self._vertex_emissives[vertex_indices[i]]) # XXX use glColorMaterial when emissive color but no diffuse ?
			if self._option & SHAPE_TEXCOORDS: glTexCoord2fv(self._values   + self._vertex_texcoords[vertex_indices[i]])
		...
		for i from 0 <= i < 3:
			...
			if p[i3 + 2] > -renderer.current_camera._front:
				...
				if self._option & SHAPE_DIFFUSES : memcpy(inter + (cur_inter3 +  3), self._colors + self._vertex_diffuses [vertex_indices[i]], 4 * sizeof(float))
				if self._option & SHAPE_EMISSIVES: memcpy(inter + (cur_inter3 +  7), self._colors + self._vertex_emissives[vertex_indices[i]], 4 * sizeof(float))
				if self._option & SHAPE_TEXCOORDS: memcpy(inter + (cur_inter3 + 11), self._values + self._vertex_texcoords[vertex_indices[i]], 2 * sizeof(float))
		...
		if cur_inter3 != 0:
			...
			for i from 0 <= i < nb_inter:
				cur_inter3 = i * 13
				if self._option & SHAPE_DIFFUSES : glColor4fv   (inter + (cur_inter3 +  3))
				if self._option & SHAPE_EMISSIVES: glMaterialfv (GL_FRONT_AND_BACK, GL_EMISSION, inter + (cur_inter3 +  7))
				if self._option & SHAPE_TEXCOORDS: glTexCoord2fv(inter + (cur_inter3 + 11))
