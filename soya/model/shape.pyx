...
cdef int face_vertices_number(ShapeFace* self):
...
cdef class _SimpleShape(_Shape):
  ...
  cdef __getcstate__(self):
    ...
    if self._option & SHAPE_DIFFUSES:       chunk_add_ints_endian_safe  (chunk, self._vertex_diffuses , self._nb_vertices)
    if self._option & SHAPE_EMISSIVES:      chunk_add_ints_endian_safe  (chunk, self._vertex_emissives, self._nb_vertices)
    if self._option & SHAPE_HAS_SPHERE:     chunk_add_floats_endian_safe(chunk, self._sphere          , 4)
    ...
    if self._option & SHAPE_NEIGHBORS:
    ...
    if self._option & SHAPE_SIMPLE_NEIGHBORS:
  ...
  cdef void __setcstate_data__(self, cstate):
    ...
    self._faces         = <ShapeFace*> malloc(    self._nb_faces    * sizeof(ShapeFace))
    ...
    if self._option & SHAPE_VERTEX_OPTIONS: self._vertex_options   = <char* > malloc(self._nb_vertices * sizeof(char )); chunk_get_chars_endian_safe (chunk, self._vertex_options  , self._nb_vertices)
    if self._option & SHAPE_TEXCOORDS:      self._vertex_texcoords = <int*  > malloc(self._nb_vertices * sizeof(int  )); chunk_get_ints_endian_safe  (chunk, self._vertex_texcoords, self._nb_vertices)
    if self._option & SHAPE_DIFFUSES:       self._vertex_diffuses  = <int*  > malloc(self._nb_vertices * sizeof(int  )); chunk_get_ints_endian_safe  (chunk, self._vertex_diffuses , self._nb_vertices)
    if self._option & SHAPE_EMISSIVES:      self._vertex_emissives = <int*  > malloc(self._nb_vertices * sizeof(int  )); chunk_get_ints_endian_safe  (chunk, self._vertex_emissives, self._nb_vertices)
    if self._option & SHAPE_HAS_SPHERE:     self._sphere           = <float*> malloc(4                 * sizeof(float)); chunk_get_floats_endian_safe(chunk, self._sphere          , 4)
    ...
    for i from 0 <= i < self._nb_faces:
      ...
      #chunk_get_ints_endian_safe(chunk, face.v, 4)
    ...
    self._option = self._option & ~SHAPE_INITED
  ...
  def get_vertex_index(self, int index):
    ...
    if self._option & SHAPE_VERTEX_OPTIONS: l.append(self._vertex_options[index])
    else:                                   l.append(-1)
    if self._option & SHAPE_TEXCOORDS:      l.append(self._vertex_texcoords[index])
    else:                                   l.append(-1)
    if self._option & SHAPE_DIFFUSES:       l.append(self._vertex_diffuses[index])
    else:                                   l.append(-1)
    if self._option & SHAPE_EMISSIVES:      l.append(self._vertex_emissives[index])
    else:                                   l.append(-1)
    return tuple(l)
  ...
  def get_vertex(self, int index):
    ...
    if self._option & SHAPE_TEXCOORDS:      l.append((self._values[self._vertex_texcoords[index]], self._values[self._vertex_texcoords[index] + 1]))
    else:                                   l.append(-1)
    if self._option & SHAPE_DIFFUSES:       l.append((self._colors[self._vertex_diffuses [index]], self._colors[self._vertex_diffuses [index] + 1], self._colors[self._vertex_diffuses [index] + 2], self._colors[self._vertex_diffuses [index] + 3]))
    else:                                   l.append(-1)
    if self._option & SHAPE_EMISSIVES:      l.append((self._colors[self._vertex_emissives[index]], self._colors[self._vertex_emissives[index] + 1], self._colors[self._vertex_emissives[index] + 2], self._colors[self._vertex_emissives[index] + 3]))
    else:                                   l.append(-1)
    return tuple(l)
  
  def get_neighbor(self, int index):
    if not (self._option & SHAPE_NEIGHBORS): return None
    cdef int* neighbor
  ...
  def get_simple_neighbor(self, int index):
    if not (self._option & SHAPE_SIMPLE_NEIGHBORS): return None
    cdef int* neighbor
  ...
  cdef void _add_face(self, _Face face, vertex2ivertex, ivertex2index, lights, int static_shadow):
    ...
    else: raise ValueError("Face with %s vertices are not supported in shape." % nb_vertices)
    ...
    face._normal._out(n)
    if self._option & SHAPE_PLANE_EQUATION:
  ...
  cdef int _add_vertex(self, _Vertex vertex, vertex2ivertex, ivertex2index, lights, int static_shadow):
    ...
    if self._option & SHAPE_DIFFUSES:
    ...
    if self._option & SHAPE_EMISSIVES:
    ...
    if self._option & SHAPE_EMISSIVES: emissive = self._register_color(value)
    else: emissive = -1
    ...
    if self._option & SHAPE_TEXCOORDS: self._vertex_texcoords = <int*> realloc(self._vertex_texcoords, self._nb_vertices * sizeof(int)); self._vertex_texcoords[i] = texcoord
    if self._option & SHAPE_DIFFUSES : self._vertex_diffuses  = <int*> realloc(self._vertex_diffuses , self._nb_vertices * sizeof(int)); self._vertex_diffuses [i] = diffuse
    if self._option & SHAPE_EMISSIVES: self._vertex_emissives = <int*> realloc(self._vertex_emissives, self._nb_vertices * sizeof(int)); self._vertex_emissives[i] = emissive
    return i
...
#           ivertices = ivertices + [vertex]
  ...
  def __init__(self, _World world, float angle, int option, lights):
    ...
    # Collect faces
    # XXX collect shapes too (by loading the corresponding world)
    faces = []
    ...
    if lights: self._option = self._option | (SHAPE_STATIC_LIT + SHAPE_EMISSIVES)
      
    for face in faces:
      for vertex in face._vertices:
        if (not face._material._texture is None) and ((vertex._tex_x != 0.0) or (vertex._tex_y != 0.0)): self._option = self._option | SHAPE_TEXCOORDS
        if not vertex._diffuse  is None: self._option = self._option | SHAPE_DIFFUSES
        if not vertex._emissive is None: self._option = self._option | SHAPE_EMISSIVES
    ...
    # creates the faces
    self._faces = <ShapeFace*> malloc(len(faces) * sizeof(ShapeFace))
    for face in faces: self._add_face(face, vertex2ivertex, ivertex2index, lights, self._option & SHAPE_STATIC_SHADOW)
    
    # avoid SimpleShape.material.append(...)
  ...
  cdef void _build_sphere(self):
    if self._nb_coords > 0:
      ...
      self._option = self._option | SHAPE_HAS_SPHERE
  ...
  cdef void _build_display_list(self):
    ...
    self._option = self._option | SHAPE_DISPLAY_LISTS
    
  cdef void _init_display_list(self):
    cdef DisplayList*  display_list
    cdef ShapeFace*    face
    cdef int i, nb, j
    #shape_option_activate(self._option) # XXX usefull ? not put in the display list, but may modify the data stored in it ???
    ...
    for i from 0 <= i < nb:
      ...
      if   display_list.option & FACE_TRIANGLE: glBegin(GL_TRIANGLES)
        ...
        print "Shape supports only triangle or quad faces !"
        raise ValueError("Shape supports only triangle or quad faces !")
  ...
  def __dealloc__(self):
    cdef DisplayList*  display_list
    cdef int i, nb
    if (self._option & SHAPE_DISPLAY_LISTS) and (self._option & SHAPE_INITED):
      nb = self._display_lists.nb_opaque_list + self._display_lists.nb_alpha_list
      display_list = self._display_lists.display_lists
      for i from 0 <= i < nb:
        display_list = self._display_lists.display_lists + i
    ...
    if self._option & SHAPE_VERTEX_OPTIONS: free(self._vertex_options)
    if self._option & SHAPE_TEXCOORDS:      free(self._vertex_texcoords)
    if self._option & SHAPE_DIFFUSES:       free(self._vertex_diffuses)
    if self._option & SHAPE_EMISSIVES:      free(self._vertex_emissives)
...
#     return
    ...
    shape_option_activate(self._option) # XXX put this in the display list ?
    
    if self._option & SHAPE_DISPLAY_LISTS:
      if not(self._option & SHAPE_INITED): self._init_display_list()
  ...
  cdef void _raypick(self, RaypickData data, CoordSyst parent):
    ...
    if (self._option & SHAPE_HAS_SPHERE) and (sphere_raypick(raydata, self._sphere) == 0): return
    for i from 0 <= i < self._nb_faces:
  ...
  cdef int _raypick_b(self, RaypickData data, CoordSyst parent):
    ...
    cdef int    i
    raydata = parent._raypick_data(data)
    if (self._option & SHAPE_HAS_SPHERE) and (sphere_raypick(raydata, self._sphere) == 0): return 0
    for i from 0 <= i < self._nb_faces:
  ...
  cdef void _collect_raypickables(self, Chunk* items, float* rsphere, float* sphere, CoordSyst parent):
    if not(self._option & SHAPE_HAS_SPHERE) or (sphere_distance_sphere(sphere, self._sphere) < 0.0):
  ...
  cdef void _render_vertex(self, int index, int face_option):
    ...
    if self._option & SHAPE_TEXCOORDS: glTexCoord2fv(self._values   + self._vertex_texcoords[index])
...
cdef void shape_option_activate(int option):
  if option & SHAPE_STATIC_LIT: disable_static_lights()
  
cdef void shape_option_inactivate(int option):
  if option & SHAPE_STATIC_LIT: enable_static_lights()
...
cdef void shape_shadow_tess_combine(double coords[3], void* vertex_data[4], float weight[4], void** out_data):
  cdef double* d
